...
:warning: **Предупреждение:** Если вашей тестовой средой является контейнер Docker, предварительно нужно войти (или выполнить команду внутри контейнера) с правами root:

```bash
docker exec -it -u 0 %имя контейнера% bash
```
...
:warning: **Предупреждение:** Если вашей тестовой средой является контейнер Docker, достаточно просто перезапустить контейнер

Вы должны увидеть успешный запуск сервера разработки. Войдите в систему как суперпользователь и перейдите в пользовательский интерфейс администратора нажав шестеренку в выпадющем меню, там где раньше была кнопка "Log in". В разделе **System > Installed plugins** вы должны увидеть список наш плагин.
