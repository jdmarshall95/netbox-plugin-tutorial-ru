...
По умолчанию экземпляры модели упорядочены по их первичным ключам, но было бы разумнее упорядочить списки доступа по имени. Мы можем сделать это, создав дочерний класс `Meta` и определив переменную `ordering`. (Обязательно создайте подкласс `Meta` *внутри* `AccessList`, а не после него.)
